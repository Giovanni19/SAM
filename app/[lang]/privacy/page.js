...
export default function PrivacyPage({ params }) {
  ...
  return (
    <div className="container-sam max-w-3xl py-12">
      {/* Il testo legale resta in italiano: tradurlo richiede una validazione
          legale della versione EN, che non è ancora stata fatta. Agli utenti in
          inglese mostriamo una nota, invece di una traduzione non validata. */}
      {lang === "en" && (
        <p className="mt-4 rounded-2xl border border-sam-cream bg-white p-4 text-sm text-sam-brown/90">
          {getDictionary("en").legal.onlyItalianNotice}
        </p>
      )}
