...
const SITE = {
  ...
  en: {
    title: "SAM — Study Areas Milan",
    description:
      "Find the best study spots, cafés, libraries and coworking spaces in Milan to study and work in.",
    ogLocale: "en_GB",
    url: "/en",
  },
};

// Genera staticamente entrambe le lingue.
...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    // hreflang: dice a Google che le due home sono la stessa pagina in lingue
    // diverse, così indicizza entrambe senza considerarle contenuti duplicati.
    alternates: {
      canonical: site.url,
      languages: { it: "/", en: "/en", "x-default": "/" },
    },
    // Default per tutte le pagine; le pagine spazio sovrascrivono con la
    // propria foto/descrizione in generateMetadata. L'immagine di default
    // arriva da app/opengraph-image.js (convenzione Next.js, via ImageResponse).
    openGraph: {
      title: site.title,
      description: site.description,
      url: site.url,
...
export default function RootLayout({ children, params }) {
  ...
  return (
    <html lang={t.htmlLang} className={`${poppins.variable} ${nunito.variable}`}>
