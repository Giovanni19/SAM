...
const it = {
  ...
  // Nomi per esteso: il grafico affollamento mostra il giorno completo nella
  // riga di dettaglio ("Lunedì 14:00–15:00"), non l'abbreviazione dei tasti.
  daysFull: {
  ...
  crowd: {
    hint: "Tocca una barra per vedere l'orario e la percentuale esatti.",
    busyness: "di affollamento",
    quiet: "poco affollato o chiuso",
    barLabel: (hour, score) => `${hour} — ${score}% di affollamento`,
    // La nota è spezzata perché due parti sono in grassetto nel rendering.
    sourceLead: "Dati storici di Google Maps",
    sourceMiddle:
      ", non in tempo reale: sono la media delle visite passate in ogni fascia oraria. La percentuale indica quanto è pieno il locale ",
    sourceStrong: "rispetto alla sua ora di punta della settimana",
    sourceTail: " (100% = massimo affollamento abituale, non capienza reale).",
  },
  ...
  auth: {
    ...
    mustAcceptPrivacy: "Per registrarti devi accettare l'Informativa privacy.",
    confirmSent: "Ti abbiamo inviato un'email di conferma: aprila per attivare l'account.",
    magicSent: "Link di accesso inviato! Controlla la tua email.",
  ...
  // Etichette delle occupazioni: i VALORI (studente, lavoratore, …) restano
  // canonici in italiano perché finiscono su Supabase — qui si traduce solo
  // ciò che si legge a schermo.
  occupations: {
    studente: "Studente",
    lavoratore: "Lavoratore",
    libero_professionista: "Libero professionista",
  },
  // Stessa regola per l'ultima voce della lista atenei: il valore salvato
  // resta "Altro".
  universityOther: "Altro",
  ...
  profile: {
    ...
    consentOff:
      "I campi occupazione, università ed età sono disattivati: puoi abilitarli dando il consenso alle analytics nella sezione \u201CPrivacy e dati\u201D qui sotto.",
  },

  privacySection: {
    title: "Privacy e dati",
    subtitlePre: "Gestisci il consenso ed esercita i tuoi diritti. Dettagli nell'",
    subtitleLink: "Informativa privacy",
    consentOn: "Consenso attivato. Ora puoi compilare i dati di profilazione.",
    consentOff: "Consenso revocato e dati di profilazione eliminati.",
    export: "⬇ Esporta i miei dati (JSON)",
    ...
    deleteConfirm: "Sei sicuro? L'azione non è annullabile.",
    deleting: "Eliminazione…",
    deleteYes: "Sì, elimina definitivamente",
    cancel: "Annulla",
    deleteError: "Non è stato possibile eliminare l'account. Riprova più tardi.",
  },

  myComments: {
    emptyTitle: "Ancora nessun commento",
    emptyHint: "Lascia un commento su uno spazio per vederlo qui.",
    emptyCta: "Esplora gli spazi",
    spaceGone: "Spazio non più disponibile",
    edited: "modificato",
    delete: "Elimina",
  },
  ...
  legal: {
    ...
    consentItalianNotice:
      "The consent wording below is a legal text and is available in Italian only. If you need help understanding it, write to info@studyareasmilan.it.",
  },
};
...
const en = {
  ...
  crowd: {
    hint: "Tap a bar to see the exact hour and percentage.",
    busyness: "busy",
    quiet: "quiet or closed",
    barLabel: (hour, score) => `${hour} — ${score}% busy`,
    sourceLead: "Historical Google Maps data",
    sourceMiddle:
      ", not live: it averages past visits in each time slot. The percentage shows how full the place is ",
    sourceStrong: "compared with its own busiest hour of the week",
    sourceTail: " (100% = its usual peak, not actual capacity).",
  },
  ...
  auth: {
    ...
    mustAcceptPrivacy: "To sign up you must accept the Privacy policy.",
    confirmSent: "We've sent you a confirmation email: open it to activate your account.",
  ...
  profile: {
    ...
    consentOff:
      "The occupation, university and age fields are switched off: you can enable them by giving analytics consent in the \u201CPrivacy and data\u201D section below.",
  },

  privacySection: {
    title: "Privacy and data",
    subtitlePre: "Manage your consent and exercise your rights. Details in the ",
    subtitleLink: "Privacy policy",
    consentOn: "Consent given. You can now fill in your profile details.",
    consentOff: "Consent withdrawn and profile details deleted.",
    export: "⬇ Export my data (JSON)",
    deleteTitle: "Delete your account",
    deleteWarning:
      "This cannot be undone: your account, profile and favourites will be permanently deleted.",
    deleteCta: "Delete account",
    deleteConfirm: "Are you sure? This cannot be undone.",
    deleting: "Deleting…",
    deleteYes: "Yes, delete permanently",
    cancel: "Cancel",
    deleteError: "We couldn't delete your account. Please try again later.",
  },

  myComments: {
    emptyTitle: "No comments yet",
    emptyHint: "Leave a comment on a space and it will show up here.",
    emptyCta: "Explore spaces",
    spaceGone: "Space no longer available",
    edited: "edited",
    delete: "Delete",
  },
...
/**
 * Le server action di autenticazione girano fuori dal contesto di lingua della
 * pagina, quindi non restituiscono testo ma un codice: qui lo trasformiamo nel
 * messaggio della lingua corrente. Un codice sconosciuto ricade sull'errore
 * generico invece di mostrare all'utente una stringa tecnica.
 */
const AUTH_MESSAGES = {
  missingCredentials: (t) => t.auth.missingCredentials,
  missingEmail: (t) => t.auth.forgotMissing,
  passwordTooShort: (t) => t.auth.resetTooShort,
  mustAcceptPrivacy: (t) => t.auth.mustAcceptPrivacy,
  confirmSent: (t) => t.auth.confirmSent,
  magicSent: (t) => t.auth.magicSent,
  invalidCredentials: (t) => t.auth.errInvalid,
  emailNotConfirmed: (t) => t.auth.errUnconfirmed,
  alreadyRegistered: (t) => t.auth.errAlreadyRegistered,
  rateLimit: (t) => t.auth.errRateLimit,
  generic: (t) => t.auth.errGeneric,
};

export function authMessage(code, t) {
  return AUTH_MESSAGES[code]?.(t) ?? t.auth.errGeneric;
}
