...
export const TYPE_META = {
  Caffetteria: { label: "Caffetteria", emoji: "☕", color: "bg-sam-brown", hex: "#5A2A20" },
  ...
  Libreria: { label: "Libreria", emoji: "📖", color: "bg-sam-orange", hex: "#E0734F" },
...
const AC = {
  presente: { tone: "good", label: "Aria condizionata" },
  "non verificato": { tone: "mid", label: "Aria condizionata non verificata" },
  assente: { tone: "bad", label: "Niente aria condizionata" },
};
...
export const AMENITY_FILTERS = [
  ...
  { key: "ac", label: "Aria condizionata", map: AC },
];
...
export function getAmenities(space) {
  const rows = [
    ...
    { key: "ac", icon: "❄️", group: "Aria condizionata", map: AC, value: space.ac },
  ];
...
export function spaceMetaDescription(space) {
  const base =
    space.description ||
    `${typeMeta(space.type).label} a Milano${space.zone ? ` in zona ${space.zone}` : ""}${
 ...
 */
export function spaceJsonLd(space, url) {
  return {
