...
// Attacca lat/lng dalla tabella COORDS (geocoding OSM) usando l'id.
function withCoords(space) {
  const c = COORDS[space.id];
  return { ...space, lat: c?.[0] ?? space.lat ?? null, lng: c?.[1] ?? space.lng ?? null };
}
...
function normalizeSpace(page) {
  ...
  return {
    ...
    studyScore: getNumber(p["Study Score"]),
    description,
...
/** Restituisce tutti gli spazi studio (ordinati per Study Score). */
export async function getSpaces() {
  if (shouldUseMock) {
    return MOCK_SPACES.map(withCoords).sort(
      (a, b) => (b.studyScore ?? 0) - (a.studyScore ?? 0)
    );
  }
  ...
  try {
    ...
    return pages
      ...
      .sort((a, b) => (b.studyScore ?? 0) - (a.studyScore ?? 0));
  } catch (err) {
